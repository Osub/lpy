...
<?php
	if(!empty($demos)){ ?>
		...
		<span class="demo">
			...
			?>
			<?php $demosuffix = ot_get_option('page_demo','demo'); if(stripos($demosuffix,'?')===false){$demopage=get_bloginfo('home').'/'.$demosuffix.'?id='.$post->ID.'_'.$i;}else{$demopage=get_bloginfo('home').'/'.$demosuffix.'&id='.$post->ID.'_'.$i;} ?>
	...
	<?php if($dlmail==0||empty($dlmail)||$dlmail==1&&!empty($saledl)){ ?>
		...
		<span class="dl-link">
			<?php $dlsuffix = ot_get_option('page_dl','dl'); if(stripos($dlsuffix,'?')===false){$dlpage=get_bloginfo('home').'/'.$dlsuffix.'?pid='.$post->ID;}else{$dlpage=get_bloginfo('home').'/'.$dlsuffix.'&pid='.$post->ID;} ?>
	...
	<?php if($dlmail==1&&!empty($dlinks)){ ?>
	<div class="dl-terms">
		<input type="checkbox" name="dl-terms" id="dl-terms-chk" /><?php _e(' 我同意','tinection'); ?>&nbsp;<a href="<?php bloginfo('home'); echo'/copyright'; ?>" title="Copyright" target="_blank"><?php _e('本站条款','tinection'); ?></a><?php _e('并愿意接收包含最新文章的订阅邮件。','tinection'); ?>
		<p class="dl-msg"></p>
