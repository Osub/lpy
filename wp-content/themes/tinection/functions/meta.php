...
/* 删除文章时删除喜欢meta
/* -------------------------------------- */
function delete_tin_likes_fields($post_ID) {
	global $wpdb;
	if(!wp_is_post_revision($post_ID)) {
		delete_post_meta($post_ID, 'tin_likes');
	}
}
add_action('delete_post', 'delete_tin_likes_fields');
...
/* 删除文章时删除投票meta
/* ------------------------ */
function delete_tin_rating_fields($post_ID) {
	global $wpdb;
	if(!wp_is_post_revision($post_ID)) {
...
add_action('delete_post', 'delete_tin_rating_fields');
...
/* 删除文章时删除收藏meta
/* ------------------------ */
function delete_tin_collect_fields($post_ID) {
	global $wpdb;
	if(!wp_is_post_revision($post_ID)) {
		delete_post_meta($post_ID, 'tin_post_collects');
	}
}
add_action('delete_post', 'delete_tin_collect_fields');
