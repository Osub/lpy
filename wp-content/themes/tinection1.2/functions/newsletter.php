...
function tin_newsletter_html_foot($type,$num){
	$blogname =  get_bloginfo('name');
	$bloghome = get_bloginfo('home');
	$newssuffix = ot_get_option('page_newsletter','newsletter'); if(stripos($newssuffix,'?')===false){$newspage=get_bloginfo('home').'/'.$newssuffix.'?';}else{$newspage=get_bloginfo('home').'/'.$newssuffix.'&';}
...
function tin_newsletter_subscribe(){
	...
	$subscribers_array = explode(',',$subscribers);
	$newssuffix = ot_get_option('page_newsletter','newsletter');$newspage=get_bloginfo('home').'/'.$newssuffix;
	...
	$type = '邮件订阅';
	$content = '<p>你已成功订阅<a href="'.get_bloginfo('home').'" target="_blank">'.get_bloginfo('name').'</a>！</p><p>我们将以周刊的形式推送最新文章到你的邮箱，欢迎你随时浏览。</p><p>你也可以现在查看往期期刊，<a href="'.$newspage.'" target="_blank" style="color:#1cbdc5;text-decoration:underline;">点击查看</a>。</p><p>感谢你对本站的支持，祝你生活愉快!</p>';
...
function tin_newsletter_unsubscribe(){
	...
	$newssuffix = ot_get_option('page_newsletter','newsletter'); if(stripos($newssuffix,'?')===false){$newspage=get_bloginfo('home').'/'.$newssuffix.'?action=unsubscribe';}else{$newspage=get_bloginfo('home').'/'.$newssuffix.'&action=unsubscribe';}
